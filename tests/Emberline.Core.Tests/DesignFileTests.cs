...
public class DesignFileTests : IDisposable
{
    private readonly string _folder = Path.Combine(
        Path.GetTempPath(), "emberline-designs", Guid.NewGuid().ToString("N"));
...
/// <summary>
/// Carrying work across the rename.
///
/// A product changing its name must not quietly invalidate the files and settings
/// somebody already has. Both of these are cheap to write and would be expensive
/// to discover missing.
/// </summary>
public class RenameMigrationTests
{
    [Fact]
    public void FilesSavedUnderTheOldNameAreStillRecognised()
    {
        Assert.True(DesignFile.IsDesign("coaster.emberline"));
        Assert.True(DesignFile.IsDesign("coaster.openburn"));
        Assert.True(DesignFile.IsDesign("/some/path/Coaster.OPENBURN"));
        Assert.False(DesignFile.IsDesign("coaster.svg"));
    }

    [Fact]
    public void AnOldFileOpensAsWellAsANewOne()
    {
        // The format did not change with the name.
        var folder = Path.Combine(Path.GetTempPath(), $"emberline-legacy-{Guid.NewGuid():N}");
        Directory.CreateDirectory(folder);
        var path = Path.Combine(folder, "old" + DesignFile.LegacyExtension);

        try
        {
            var design = new Design { Name = "From before the rename" };
            design.Layers.Add(Layer.CreateDefault(OperationKind.Cut, 0));
            DesignFile.Save(design, path);

            var reopened = DesignFile.Load(path);
            Assert.Equal("From before the rename", reopened.Name);
        }
        finally
        {
