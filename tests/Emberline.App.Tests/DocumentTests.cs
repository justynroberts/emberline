...
public class DocumentTests : ShellTest
{
    private static string Scratch(string name) =>
        Path.Combine(Path.GetTempPath(), $"emberline-doc-{Guid.NewGuid():N}-{name}{DesignFile.Extension}");
    ...
    {
        // Dropping a .emberline file on the window must open it, not try to trace it.
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"emberline-bad-{Guid.NewGuid():N}{DesignFile.Extension}");
