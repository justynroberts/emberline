...
using Emberline.Plugins;
using Emberline.Transport;
using Xunit;

namespace Emberline.Plugins.Tests;

/// <summary>A plugin that registers one of everything.</summary>
public sealed class GoodPlugin : IEmberlinePlugin
{
...
/// <summary>A plugin that tries to take names Emberline already owns.</summary>
public sealed class GreedyPlugin : IEmberlinePlugin
{
...
public sealed class BrokenPlugin : IEmberlinePlugin
{
...
public class PluginRegistryTests
{
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "emberline-plugin-tests", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            Assert.False(report.AnythingLoaded);
            Assert.Contains(report.Failures, f => f.Contains("IEmberlinePlugin", StringComparison.Ordinal));
        }
        finally
        {
...
public class PluginFileLockTests
{
    ...
    public void LoadingAPluginDoesNotLockItsFile()
    {
        ...
        // while Emberline is running and the operator gets "access denied" with
        ...
        var folder = Path.Combine(Path.GetTempPath(), "emberline-plugin-lock", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(folder);
