...
///
/// Until this existed Emberline could export G-code and nothing else, which meant a
...
{
    public const string Extension = ".emberline";

    /// <summary>
    /// The extension this format used before the application was renamed. Still
    /// opened, because a file somebody saved last week is not made invalid by a
    /// change of product name.
    /// </summary>
    public const string LegacyExtension = ".openburn";

    /// <summary>Is this a design file, whichever name it was saved under?</summary>
    public static bool IsDesign(string path) =>
    ...
    public static Design Load(string path)
    {
        var document = JsonSerializer.Deserialize<DesignDocument>(File.ReadAllText(path), Options)
            ?? throw new InvalidDataException("That file is not an Emberline design.");

        if (document.Version > CurrentVersion)
        {
            throw new InvalidDataException(
                $"That design was saved by a newer version of Emberline (format {document.Version}, this build reads {CurrentVersion}).");
        }
