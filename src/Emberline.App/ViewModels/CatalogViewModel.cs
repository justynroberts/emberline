...
public sealed partial class CatalogViewModel : ObservableObject
{
    ...
    private async Task SearchAsync()
    {
        ...
        {
            Status = $"Could not reach the catalogue: {ex.Message}. Everything else in Emberline works offline.";
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Draw the preview with Emberline's own importer.
    ///
    /// Deliberately not a general SVG renderer. What matters is not how the icon
    /// looks in a browser but what Emberline will actually put on the bed, and the
