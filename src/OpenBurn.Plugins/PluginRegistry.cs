...
public sealed class PluginRegistry : IPluginRegistry
{
    ...
    /// <summary>Names that plugins may not take, because OpenBurn already provides them.</summary>
    ...
    public void AddDeviceDriver(string driverId, string description, Func<MachineProfile, ILaserDevice> factory)
    {
        ...
        {
            Log($"refused to register the driver '{driverId}' — that name belongs to OpenBurn.");
            return;
        }
    ...
    public void AddImporter(string name, IReadOnlyList<string> extensions, Func<string, IReadOnlyList<Shape>> importer)
    {
        var usable = extensions
            ...
            .Where(e =>
            {
                ...
                Log($"refused '{e}' from the importer '{name}' — OpenBurn already reads that format.");
                return false;
            })
            .Where(e =>
            {
