    ...
    public static List<SettingWarning> Audit(IReadOnlyDictionary<int, double> values)
    {
        ...
        if (Get(30) is { } maxS && Math.Abs(maxS - 1000) > 0.5)
        {
            warnings.Add(new SettingWarning(30, false,
                $"Maximum spindle speed ($30) is {maxS:0}. OpenBurn scales power to this value — make sure the machine profile uses the same number."));
        }
        ...
        if (Get(13) == 1)
        {
            warnings.Add(new SettingWarning(13, true,
                "The controller is reporting in inches ($13=1). OpenBurn expects millimetres — set $13=0."));
        }
