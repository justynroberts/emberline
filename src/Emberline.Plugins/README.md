# Writing an Emberline plugin

A plugin is a .NET class library that references `Emberline.Plugins` and exposes one
type implementing `IEmberlinePlugin`:

```csharp
using Emberline.Plugins;

public sealed class MyPlugin : IEmberlinePlugin
{
...
Build it and drop the output in `plugins/` inside the Emberline application data
...
A plugin is ordinary code running in the Emberline process with your permissions.
