...
public static class PluginHost
{
    ...
    public static PluginLoadReport Load(PluginRegistry registry, string? directory = null, bool enabled = true)
    {
        ...
        foreach (var path in FindCandidates(folder))
        {
            try
            {
                ...
                if (found.Count == 0)
                {
                    failures.Add($"{Path.GetFileName(path)}: no type implementing IEmberlinePlugin.");
                    continue;
                }
    ...
    public static IReadOnlyList<LoadedPlugin> RegisterFrom(Assembly assembly, PluginRegistry registry, string source)
    {
        ...
        foreach (var type in types)
        {
            ...
            if (!typeof(IEmberlinePlugin).IsAssignableFrom(type)) continue;
            if (type.GetConstructor(Type.EmptyTypes) is null) continue;

            try
            {
                if (Activator.CreateInstance(type) is not IEmberlinePlugin plugin) continue;
    ...
    /// beside it while sharing Emberline's own assemblies with the host — otherwise
    ...
    private sealed class PluginLoadContext : AssemblyLoadContext
    {
        ...
        {
            // Anything Emberline already has loaded must be shared, not duplicated.
            if (assemblyName.Name?.StartsWith("Emberline", StringComparison.Ordinal) == true) return null;
